...
class User < ActiveRecord::Base 
  ...
  @@search_columns = ["users.firstname", "users.surname", "users.grad_year", "users.phone", "users.email"]
  ...
  def as_json(options={})
    json = {
      ...
      :jobs => self.jobs,
      :phone => self.phone,
      :login => self.user_detail.login,
      :level => self.user_detail.level,
  ...
  def to_xml(options={})
    ...
    xml.user do
      ...
      xml.grad_year self.grad_year
      xml.firstname self.firstname
      xml.surname self.surname
      xml.jobs self.jobs, :type => 'boolean'
      xml.phone self.phone
      xml.login self.user_detail.login
      xml.level self.user_detail.level
